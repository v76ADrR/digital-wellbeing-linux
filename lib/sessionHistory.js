...
/**
 * GNOME Settings → Wellbeing does not sample apps, websites, or keystrokes.
 * It does not measure screen time itself. GNOME Shell’s TimeLimitsManager does.
 *
 * Shell watches logind: at least one session in State "active", IdleHint false,
 * and the machine not preparing for sleep (same idea as sd_uid_get_state()
 * “active”). Lock counts as idle. On every logind property change and
 * prepare-for-sleep, Shell compares previous vs new state. On a flip it
 * appends an edge {oldState, newState, wallTimeSecs} (0=INACTIVE, 1=ACTIVE).
 *
 * History is only those edges: ~/.local/share/gnome-shell/session-active-history.json
 * (global.userdatadir). Not a timesheet. Shell keeps ≤14 weeks, drops future
 * timestamps, flushes on change / inhibit-suspend / shutdown.
 *
 * Settings only reads/writes org.gnome.desktop.screen-time-limits (history-enabled,
 * daily-limit-*, grayscale) and sums the file for the graph. Break reminders are
 * BreakManager + Mutter idle monitor (D-Bus org.gnome.Shell.ScreenTime), not this
 * graph. malcontent-timerd is a separate parental path; when those limits are off,
 * the Wellbeing graph is still logind + this JSON.
 *
 * This module does the same sum Settings does: walk ACTIVE intervals, and if the
 * last edge left the session ACTIVE, add now − last ACTIVE start. Daily limit and
 * greyscale in Shell use that same sum. We do not write the file.
 */
const USER_ACTIVE = 1;
...
/**
 * TimeLimitsManager’s “today” is not calendar midnight. To avoid the missing
 * 24:00–01:00 hour on DST spring-forward, Shell starts the wellbeing day at
 * 03:00 local. Usage from 00:00–03:00 belongs to the previous date key.
 */
